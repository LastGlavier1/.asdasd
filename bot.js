      ...
      if(üs) {
        üyesayısı = üyesayısı.replace(/([0-9])/g, d => {
          return {
            ...
            '8': `8`,
            '9': `9`}[d];})}
    const kanal = member.guild.channels.cache.find(r => r.id === "840009967783116850"); // HOŞGELDİNİZ KANAL İD
...
Seninle birlikte **`+üyesayısı+`** kişiye ulaştık ! Tagımızı (\`${tag}\`) alarak bizlere destek olabilirsin ! Kayıt olmak için teyit odalarına girip ses teyit vermen gerekiyor yetkililerimiz seninle ilgilenecektir. İyi eğlenceler.<@&834208105545859073>`)});
...
client.on("guildMemberAdd", member => {
  ...
  moment.locale("tr")
   ...
   x = x.replace("birkaç saniye önce", "840009855049793546")
   if(!x.includes("önce") || x.includes("sonra") ||x == "840009854240030730") {
...
setTimeout(() => {
    ...
    client.on("userUpdate", async (message, member) => {
      var sunucu = client.guilds.cache.get('834208105519644702'); // SUNUCU İD
      var uye = sunucu.members.cache.get(member.id);
      var tag = "✩"; // TAG
      var tagrol = "834208105541664880"; // TAG ROL İD
      var logKanali = "840010012294119444"; // LOG KANAL İD
    ...
    client.on("guildMemberAdd", member => {
      let tag = "✩"; //TAG
      let rol = "834208105541664880"; // TAG ROL İD
    ...
    member.roles.add(rol)
      const tagalma = new Discord.MessageEmbed()
          ...
          .setTimestamp()
         client.channels.cache.get('840010012294119444').send(tagalma) // TAG LOG KANAL İD
    }
    })
